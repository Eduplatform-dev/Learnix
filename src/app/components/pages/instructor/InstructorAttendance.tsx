...
import {
  CheckCircle, X, AlertTriangle, Users, Save, BarChart3,
  BookOpen, Calendar, Clock, ChevronDown, Eye
} from "lucide-react";
import {
  markAttendance, getCourseReport, getCourseAttendanceSessions,
  type AttendanceRecord, type AttendanceStatus, type StudentReport, type CourseReport
...
export function InstructorAttendance() {
  ...
  useEffect(() => {
    if (!selected) return;
    // Initialize records from enrolled students
    const enrolled = (selected as any).enrolledStudents || [];
    setRecords(
      enrolled.map((s: any) => ({
        student: typeof s === "string" ? s : s._id,
        ...
        <>
          ...
          <Card>
            <CardContent className="p-5">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <Label>Date *</Label>
                  <Input type="date" className="mt-1" value={date}
                ...
                <div>
                  ...
                  <Input className="mt-1" placeholder="e.g., Arrays and Pointers"
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">Student {idx + 1}</p>
                        <p className="text-xs text-gray-400 font-mono">{String(record.student).slice(-8)}</p>
                      </div>
                    </div>
                    <button
          ...
          {saved ? (
            ...
            >
              ...
              {saving ? "Saving..." : `Save Attendance — ${new Date(date).toLocaleDateString("en-IN")}`}
            </Button>
          )}
        </>
      )}
      ...
      {view === "report" && (
        <>
          {loadingReport ? (
                      ...
                      <tbody className="divide-y">
                        {[...report.students].sort((a,b) => a.percentage - b.percentage).map(s => (
                          <tr key={s.student._id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              {s.shortage
                                ? <Badge className="bg-red-100 text-red-700 text-xs"><AlertTriangle className="w-3 h-3 mr-1" />Shortage</Badge>
            ...
            <div className="text-center py-10 text-gray-400 text-sm">No report data available.</div>
          )}
        </>
      )}
    </div>
